...
//			stringer.value(game.getScores().getScore(i));
		...
		synchronized (world) {
			...
			//bullet
			/*stringer.key("bullets").array();
			for (GameEntity p : world.getEntities()) {
				if(p instanceof Projectile)
				{
					...
					stringer.value(p.id());
					stringer.key("lati");
					stringer.value(df.format(converter.getLatFromY(((Projectile) p).lastPosition.y())));
					stringer.key("longi");
					stringer.value(df.format(converter.getLongFromX(((Projectile) p).lastPosition.x())));
					stringer.endObject();
				}
			}
			stringer.endArray();
